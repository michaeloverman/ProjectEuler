...
// ProjectEuler
// Created: 6/13/20 5:05 PM

// Draft 1: ~7058676 nanos
// Draft 2: ~10062612 nanos - more concise (rows & cols) but slower...

fun main() {
    val t = measureNanoTime {
        println(largestGridProduct(testGrid) == 14169081)
        println(largestGridProduct(grid) == 70600674)
    }
    println("Time: $t")
}

fun largestGridProduct(g: List<List<Int>>): Int {
    var max = 0
    // rows
    (g.indices).forEach { i ->
        (0..g[i].size-4).forEach { j ->
//            var p = g[i][j]
//            var q = g[j][i]
//            (1..3).forEach { x ->
//                p *= g[i][j+x]
...
//            if (p > max) max = p
//            if (q > max) max = q
            if (g[i][j] * g[i][j+1] * g[i][j+2] * g[i][j+3] > max) max = g[i][j] * g[i][j+1] * g[i][j+2] * g[i][j+3]
        }
    }
    // columns
    (0..g.size-4).forEach { i ->
        (g[i].indices).forEach { j ->
            if (g[i][j] * g[i+1][j] * g[i+2][j] * g[i+3][j] > max) max = g[i][j] * g[i+1][j] * g[i+2][j] * g[i+3][j]
        }
    }

    // diagonals - down L to R
    (0..g.size-4).forEach { i ->
        (0..g[i].size-4).forEach { j ->
            if (g[i][j] * g[i+1][j+1] * g[i+2][j+2] * g[i+3][j+3] > max) max = g[i][j] * g[i+1][j+1] * g[i+2][j+2] * g[i+3][j+3]
        }
    }

    // diagonal - up L to R
    (3 until g.size).forEach { i ->
        (0..g[i].size-4).forEach { j ->
            if (g[i][j] * g[i-1][j+1] * g[i-2][j+2] * g[i-3][j+3] > max) max = g[i][j] * g[i-1][j+1] * g[i-2][j+2] * g[i-3][j+3]
        }
    }
...
val testGrid = listOf(
listOf(40, 17, 81, 18, 57),
listOf(74, 4, 36, 16, 29),
listOf(36, 42, 69, 73, 45),
listOf(51, 54, 69, 16, 92),
listOf(7, 97, 57, 32, 16)
)
